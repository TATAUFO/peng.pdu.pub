...
尽管系统中的每条信息都有明确的来源标识，但信息获取者无法仅凭单条信息进行判断。例如，看到“推荐品尝M大街上的A餐馆”或“劝各位远离N大街上的B餐馆”这样的信息，单独看都没有问题。然而，如果在短时间内看到成百上千次类似的信息，就会被认为是垃圾信息发布（spam posting）。在中心化平台中，这种情况会受到平台的过滤，并对相关账户进行限制或封禁，以确保正常信息不被淹没。

因此，我们需要一种方法，能够在必要时了解账户的相关信息，能够确保其信息未被修改。这样，我们可以根据账户地址对其进行屏蔽，避免受到垃圾信息的影响。此外，还需要一种用户过滤方式，防止可见用户范围随意增加，确保垃圾信息发布者不能通过频繁更换私钥地址在被屏蔽后继续重复上述行为。


## 发布者


在传统的平台中，通常是用户，但在这里，我们将其分为发布者或获取者两个身份，其功能的组合等同于原来的用户，但并不是一对一的组合
...
如此，使用相同私钥签名的所有消息可以通过相互引用的方式确保构成全序列，确保任意两条消息均有明确的先后顺序。使用不同私钥签名的消息可以通过相互引用，

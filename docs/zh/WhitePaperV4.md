...
在区块链系统中，所有矿工协作生成主链。主链出现分叉的情况仅是矿工之间的竞争行为，并非错误，因此系统只能通过挖矿奖励激励矿工。在这里，由于无法伪造私钥签名，一旦某账户的消息链表中出现分叉，则被视为该账户故意修改历史数据，属于主观恶意行为。类似地，如果一条消息引用列表中的第一个签名指向的不是当前私钥签名的消息，也被视为主观恶意行为。然而，如果发现某条消息的引用列表中的首条消息未被找到，则暂时保存该消息，不采信该条消息也不惩罚信息发布者。信息发布者有责任保存有自己签名的完整消息链表，以备在网络中某条消息丢失时向外部提供。
...
在传统的中心化社交平台中，即便只是浏览信息，也需要先注册一个用户身份。这种第三方平台的限制使得使用者下意识地将信息获取功能与信息发布者的身份绑定起来。平台允许用户设置不同权限来选择信息的接收者，进一步强化了这种绑定的认知。

然而，在点对点的分布式系统中，身份的定义仅限于信息发布者身份，与信息的获取过程无关。系统中的所有信息都是公开的，任何人都可以自由获取。即便经过加密的信息，也会被简单地处理为公开信息。但由于只有被指定的接收者能够通过解密理解其内容，这类信息在传播过程中无法吸引其他发布者互动行为，因此缺乏传播动力。这种消息更适合用于点对点的直接通信。
...
但在这里，我们将其分为发布者或获取者两个身份，其功能的组合等同于原来的用户，但并不是一对一的组合

。。。


使用相同私钥签名的所有消息可以通过相互引用的方式确保构成全序列，确保任意两条消息均有明确的先后顺序。这种类似区块链的结构，为了保证数据不可篡改，如果要更改一条以前的数据，则必须修改从修改的一点开始后面的所有数据。

但因为在一个点对点的分布式系统中，所有的信息是持续的公开的，这也就意味着系统中存在观察者可能会保存修改之前的数据，而这种矛盾的数据出现，必然是由于发布者主观作行为才可能发生，所以可以因此而屏蔽这个账号，认为者并非一个诚实账户。

在区块链中我们不会对于最长链出现分支二惩罚矿工，因为分支的存在不是任何一方的过错。


构成一个或多个偏序关系，这些关系可以用有向无环图（DAG）表示。

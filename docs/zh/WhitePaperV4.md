...
一个完全点对点的社交网络系统应允许使用者自由的发布信息，有效的获取信息，且过程中无需依赖任何第三方服务的帮助。然而如果这个系统因为没有采用如手机号或邮件等中心化的验证手段，使得可以零成本批量创建虚账户，则会导致系统奖惩机制失效，正常内容被垃圾信息淹没，系统无法使用。本文提出了一种在不依赖任何第三方用户认证服务的情况下，解决点对点网络中垃圾信息泛滥的方案。通过由相同私钥签名的消息形成全序列来定义信息发布者身份。转发、评论、点赞等互动行为消息中包含目标消息的引用，通过这类引用关系建立发布者之间的关联。基于上述关联关系，使用者可以按照自定义规则建立可见发布者身份集合，并在相对固定的范围内，通过基于身份奖惩实现信息的有效筛选。


## 简介

现今网络上的信息传播和交互主要依赖Facebook、Twitter/X、WeChat等中心化平台，这些平台的存在使得用户可以方便地发布信息，建立联系，平台使用算法来检测和过滤垃圾信息，确保用户体验。然而，随着这些平台的发展，中心化社交服务的问题也逐渐显现。第三方服务有可能滥用用户信息或导致数据泄露。可能会利用其强大的用户基础来锁定用户，维护其垄断地位。此外，中心化服务还容易受到政府的监管和封锁，因为它们是可控的实体。

尽管存在以上问题，我们仍然不得不继续使用这些平台。对于大多数人而言，更换平台虽不会导致数据丢失，但会失去在该平台上积累的用户关系，从而降低自身的影响力。这个原因使得用户很大程度上上被平台锁定。
...
还存在一些以区块链为基础的去中心化社交平台如Steemit和Minds。他们以一定数量的代币作为创建或者激活账户的成本，同时也用代币作为社交行为的激励。这种方式看似赋予了创建账户的成本，但其与中心化平台的成本有根本的不同。当单一账户所能带来的利益超过创建成本时，这种限制方式会完全失效。

本文提出了一种新的点对点的社交网络系统。该系统不依赖任何第三方服务的参与。任何具有相同私钥签名且具有全序关系的消息序列都被系统承认为合法的发布者身份。通过消息之间的互动，发布者身份之间能够形成公开的关联关系。任何使用者都可以根据这种关联关系，按自定义规则维护可见的发布者身份集合，并在此范围内以发布者的身份为标识，对信息进行有效的过滤。

## 消息

消息被定义为系统中的基本数据结构，并且是点对点网络中传递的唯一信息类型。系统中的其他数据类型，如发布者身份等，都是基于这种公开数据由各个节点中自行生成。

每条消息由三个部分组成：消息内容、引用列表及签名。消息内容是消息的主体，分为内容信息和互动方式两部分，前者是文字图片等内容，后者包括发布、回复、引用、点赞等常见社交网络互动形式。引用列表可包含多条消息的签名，以表示当前消息与被引用消息的时序关系。当消息内容涉及回复或引用等互动类型时，相关消息的签名应包含在引用列表中。为提供更精确的时序验证，还建议在引用列表中至少包含一条最新消息的签名。最终，将消息内容和引用列表合并计算出哈希值，并使用当前发布者的私钥对哈希值进行签名，以确认发布者身份和保证数据完整性。

尽管每条消息都有明确的内容，但我们往往无法仅凭单独的消息判断其是否为垃圾信息。例如，看到“推荐品尝M大街上的A餐馆”或“劝各位远离N大街上的B餐馆”这样的信息时，单独来看没有问题。然而，如果在短时间内出现数千次雷同的信息，就会被认为是垃圾信息发布。在中心化平台中，这种情况会受到平台的过滤，并对发布内容的账户进行限制或封禁，以确保正常信息不被其淹没。

因此，我们需要一种方法，能够在需要时了解账户发布的全部信息，并确保其发布内容未被篡改。这样有助于对发布者进行评判，以确定其发布的内容是否正常，是否需要屏蔽这个发布者来减少垃圾内容的出现。此外，还需要一种账户过滤方式，防止新账户快速的增加，确保垃圾信息发布者不能通过频繁更换私钥的方式，在被屏蔽后继续重复上述行为。


## 发布者

为了对账户进行准确判断，我们需要确保账户发布的内容未被篡改，并能在发布者删除已发布消息时进行检测。在点对点分布式系统中实现这一特性，我们要求每个账户发布的内容形成类似区块链的链表结构。规定每条消息的引用列表中，第一条引用必须是同一私钥签发的上一条信息的签名。如果当前信息是此私钥所签名的第一条信息，则引用列表中的第一个值为0，以标示起点。

在区块链系统中，所有矿工协作生成单一主链。主链出现分叉的情况仅是矿工之间的竞争行为并非错误，因此系统只能通过挖矿奖励的方式激励矿工，并通过共识机制让全系统达成一致。在本系统中，消息的签名属于个人行为，由于签名无法伪造，一旦某发布者的消息链表中出现分叉，则被视为该发布者故意修改历史数据，属于主观恶意行为。如果引用列表中的第一个签名指向的不是当前私钥签名的消息，也被视为主观恶意行为。然而，如果发现引用列表中的首条消息未被找到，则暂时保存该消息，不采信该条消息也不惩罚信息发布者。信息发布者有责任保存有自己签名的完整消息链表，以备在网络中某条消息丢失时向外部提供查验。

另一种情况是，发布者可能会隐藏分叉的消息链表，待到一个分叉中已经有多条消息被接收，再将另一个分叉的消息发布到系统中。在这种情况下，系统中已接受的消息将被视为既定事实。对发布者的惩罚通常是在发现冲突后丢弃冲突部分中后接收到的消息，并屏蔽该发布者的所有后续消息。这样的处理方式使得分叉攻击对其他人失去意义。
...
| 服务      | 说明
|----------|----------------------
|信息检索   |基于公开的消息，具备尽可能大的可见用户范围，对外提供消息检索接口。
